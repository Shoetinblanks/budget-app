...
import { Profile, Category, Account, Expense, Transaction, IncomeSource, CategoryRule, DEFAULT_CATEGORIES, PRESET_COLORS, calcAmounts, getBaseAmount, fmt } from '@/app/types'
import {
  getDashboardData,
  saveExpense,
  deleteExpense,
...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      if (data.accounts?.[0]) setExpenseForm(prev => ({ ...prev, account_code: data.accounts[0].account_code }))
      if (data.categories?.[0]) setExpenseForm(prev => ({ ...prev, category: data.categories[0].name }))
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  const handleUndoLastImport = async () => {
    ...
    try {
      ...
      if (res.count > 0) {
        alert(`Successfully removed ${res.count} transactions.`)
        loadDashboardData()
      } else {
        alert('No recent transactions found to undo.')
      }
    } catch (err: unknown) {
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to undo import.'))
    } finally {
      setIsUndoing(false)
    }
  }
  ...
  const handleInlineCategoryChange = async (t: Transaction, newCategory: string) => {
    ...
    let ruleIdToUpdate: string | undefined = undefined
    if (matchingRule && matchingRule.category !== newCategory) {
      if (confirm(`Update existing rule for "${matchingRule.merchant_pattern}" to ${newCategory}?`)) {
        ruleIdToUpdate = matchingRule.id
        setCategoryRules(categoryRules.map(r => r.id === matchingRule.id ? { ...r, category: newCategory } : r))
      }
    }
    try {
      await updateTransactionCategory(t.id, newCategory, ruleIdToUpdate)
    } catch (err) {
  ...
  const handleImportTransactions = async () => {
    ...
    setImportLoading(true)
    try {
      const res = await importTransactions(importPreview)
      if (res.importedCount > 0) {
        alert(`Imported ${res.importedCount} transactions. (${res.skippedCount} duplicates skipped)`)
      } else {
        alert('No new transactions to import. All duplicates.')
      }
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    const expenseData = { 
      ...
      name: expenseForm.name, frequency: expenseForm.frequency,
    ...
    try {
      ...
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to save expense.'))
    }
  }
  ...
  const handleSaveNewCategoryInline = async () => {
    ...
    try {
      const res = await saveCategory({ name: newCatName.trim(), color: newCatColor })
      if (res.category) {
        setCategories(prev => [...prev, res.category!])
        setExpenseForm(f => ({ ...f, category: res.category!.name }))
        setNewCatName('')
  ...
  const handleSaveIncome = async (e: React.FormEvent) => {
    ...
    if (!user) return
    const incData = {
      ...(editingIncome?.id ? { id: editingIncome.id } : {}),
      ...incomeForm,
      gross_amount: parseCurrencyInput(incomeGrossStr),
      net_amount: parseCurrencyInput(incomeNetStr),
      employer_name: incomeForm.employer_name || 'Income',
    }
    try {
      await saveIncomeSource(incData)
      setIsIncomeModalOpen(false)
      loadDashboardData()
    } catch (err: unknown) {
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to save income.'))
    }
  }
  ...
  const handlePasteImport = async () => {
    ...
    while (i < lines.length) {
      ...
      if (dateMatch) {
        ...
        results.push({ employer_name: 'Paycheck', pay_frequency: 'bi-weekly', pay_date: localDate, gross_amount: gross, net_amount: net })
      } else { i++ }
    }
  ...
  const handleConfirmPasteImport = async () => {
    ...
    try {
      await importIncomeSources(
        toInsert.map(r => ({
          employer_name: r.employer_name || 'Paycheck',
          pay_date: r.pay_date,
      ...
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to import paychecks.'))
    }
  }
  ...
  const handleSaveAccount = async (e: React.FormEvent) => {
    ...
    if (!user) return
    const accData = {
      ...(editingAccount?.id ? { id: editingAccount.id } : {}),
    ...
    try {
      ...
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to save account.'))
    }
  }
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to save category.'))
    }
  }
  ...
  const handleApplyRulesToAll = async () => {
    ...
    const updates = transactions.map(t => {
      const rule = categoryRules.find(r => t.description.toLowerCase().includes(r.merchant_pattern.toLowerCase()))
      if (rule && rule.category !== t.category && t.id) { updateCount++; return { id: t.id, category: rule.category } }
      return null
    }).filter(Boolean) as { id: string; category: string }[]

    if (updates.length > 0) {
      try {
        await batchUpdateTransactionsCategory(updates)
        alert(`Updated ${updateCount} transactions!`)
        loadDashboardData()
      } catch (err: unknown) {
        alert('Error: ' + (err instanceof Error ? err.message : 'Failed to apply rules.'))
      }
  ...
  const handleSaveRule = async (e: React.FormEvent) => {
    ...
    try {
      await saveCategoryRule(ruleData)
      setIsRuleModalOpen(false)
      loadDashboardData()
    } catch (err: unknown) {
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to save rule.'))
    }
  }
  ...
  const del = async (table: string, id: string) => {
    ...
    try {
      if (table === 'expenses') await deleteExpense(id)
      else if (table === 'accounts') await deleteAccount(id)
      else if (table === 'categories') await deleteCategory(id)
      else if (table === 'category_rules') await deleteCategoryRule(id)
      else if (table === 'income_sources') await deleteIncomeSource(id)
      loadDashboardData()
    } catch (err: unknown) {
      alert('Error: ' + (err instanceof Error ? err.message : 'Failed to delete.'))
    }
